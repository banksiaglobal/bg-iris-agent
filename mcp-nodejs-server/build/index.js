...
const logger = {
    debug: (...args) => config.logLevel === 'debug' && console.error('[DEBUG]', ...args),
    info: (...args) => ['debug', 'info'].includes(config.logLevel) && console.error('[INFO]', ...args),
    warn: (...args) => ['debug', 'info', 'warn'].includes(config.logLevel) && console.error('[WARN]', ...args),
    error: (...args) => console.error('[ERROR]', ...args)
};
async function main() {
    logger.info("Starting Banksia Iris Agent MCP bridge server");
    // Create MCP server
    const server = new McpServer({ name: "banksia-iris-agent-bridge", version: "1.0.0" }, { capabilities: { tools: {} } });
...
function registerTools(server, openApiDefinition) {
    ...
    for (const [path, methods] of Object.entries(paths)) {
        ...
        server.tool(operationId, `${summary} ${description}`.trim(), { params: convertOpenApiSchemaToZod(inputSchema) }, async (params, extra) => {
